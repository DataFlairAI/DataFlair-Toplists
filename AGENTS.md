# DataFlair Toplists — Codex Instructions
...
```bash
rsync -av --exclude='.git' --exclude='.Codex' --exclude='node_modules' --exclude='.phpunit.result.cache' \
...
8. **Rsync to strike-odds.test**:
   ```bash
   rsync -av --exclude='.git' --exclude='.Codex' --exclude='node_modules' \
     /Users/mexpower/Sites/DataFlair-Toplists/ \
     /Users/mexpower/Sites/strike-odds/wp-content/plugins/DataFlair-Toplists/
   rsync -av --exclude='.git' --exclude='.Codex' --exclude='node_modules' \
     /Users/mexpower/Sites/DataFlair-Toplists/ \
