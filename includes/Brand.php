...
class Brand
{
    ...
    public static function whereJson($jsonColumn, $jsonPath, $value)
    {
        self::$query_where[] = array(
            'type' => 'json',
            'column' => $jsonColumn,
    ...
    public static function getResults()
    {
        ...
        if (!empty(self::$query_where)) {
            foreach (self::$query_where as $where) {
                if (isset($where['type']) && $where['type'] === 'json') {
                    $where_clauses[] = "JSON_UNQUOTE(JSON_EXTRACT({$where['column']}, %s)) = %s";
                    $where_values[] = '$.' . ltrim($where['path'], '$.');
                    $where_values[] = (string) $where['value']; // force string comparison
                } else {
