...
<ul>
  ...
  <li><strong>Fixed</strong> (found only by a live delivery against a real WordPress install, not the mocked test suite): the entire REST API was fatally erroring on every request &mdash; <code>rest_bootstrap()</code> wired the webhook receiver&rsquo;s toplist-persistence dependency to the wrong class. Same root cause as the 2.3.3 <code>TestsRunner</code> incident below: this god-class wiring is deliberately outside PHPUnit&rsquo;s reach.</li>
...
<ul>
  ...
  <li><strong>Fixed:</strong> the 404 error message no longer shows a stale URL alongside the real one that failed. The Dashboard health tile, Tools diagnostics, and Test Connection now all agree with Settings about whether the API is configured. The Tools &rarr; API Preview tab no longer writes an option on a plain page load.</li>
  <li><strong>Changed:</strong> the <code>/api/vN</code> rewrite has one owner, <code>UrlTransformer::withApiVersion()</code> (brand sync, conservative) plus <code>forceApiVersion()</code> (the preview tool, which must guarantee a version). <code>ApiBaseUrlDetector::detect()</code> accepts <code>$persist = false</code> so rendering Settings never writes the cache-back option. <code>isConfigured()</code> decides when nothing is configured, so the hard-coded fallback host is never shown as the site&rsquo;s own setting, and is now shared across Settings, the sync guards, the Dashboard tile, Tools, and Test Connection. <code>SettingsPage</code> takes a single closure; two it never called were removed.</li>
  <li><strong>Tests:</strong> <code>effectiveBase()</code> for both v1&harr;v2 directions (mutation-verified), <code>detect(false)</code> never calling <code>update_option</code> (mutation-verified), <code>isConfigured()</code>, <code>withApiVersion()</code> and <code>forceApiVersion()</code> cases (mutation-verified), and new guard tests for all four sync-trigger handlers.</li>
</ul>
<h4>2.3.2</h4>
<ul>
